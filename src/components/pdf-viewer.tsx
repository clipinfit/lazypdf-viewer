...
export function PDFViewerComponent({ pdfUrl }: PDFViewerProps) {
  ...
  useEffect(() => {
    ...
    const loadingTask = pdfjsLib.getDocument({
      url: pdfUrl,
      // Enable HTTP range requests for optimized loading with Cloudflare R2
      disableRange: false, // Allow range requests
      disableStream: false, // Allow streaming
      disableAutoFetch: false, // Enable progressive loading for linearized PDFs
      rangeChunkSize: 65536, // 64KB chunks - optimal for R2
      withCredentials: false,
    });

    // Track loading progress for progress bar
    ...
    loadingTask.promise
      .then((pdf) => {
        ...
        setLoading(false);
        // Don't set progress to 100 here - let onProgress handle it
        // The PDF is viewable but download might not be complete yet
      })
      .catch((err) => {
  ...
  }, [pdfUrl, scale, knownZoomValues, percentageValues]);
  ...
  return (
    <div className="pdf-viewer">
      <div className="pdf-toolbar">
        ...
        {!error && (
          <>
            {/* Left section: Navigation controls - always visible, disabled while loading */}
            ...
            {/* Center section: Progress bar - centered when PDF is viewable */}
            {!isDownloadComplete && (
              <div className="flex items-center gap-2 flex-1 justify-center max-w-md mx-auto">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <Progress value={loadProgress} className="flex-1" />
                  <span className="text-sm text-gray-600 font-medium min-w-[3ch] tabular-nums flex-shrink-0">
                    {loadProgress}%
                  </span>
                </div>
              </div>
            ...
            {/* Right section: Zoom controls - always visible, disabled while loading */}
            <div className="flex items-center gap-2 flex-shrink-0">
              <Button
                type="button"
                variant="ghost"
